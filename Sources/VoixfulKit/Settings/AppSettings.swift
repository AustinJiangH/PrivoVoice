...
// disk on the next runloop tick (debounced). Kept in VoixfulKit (not the app
...
public final class AppSettings {
    ...
    /// `~/Library/Voixful/Models` — the documented default install location.
    public static var defaultModelsDirectory: URL {
        let home = FileManager.default.homeDirectoryForCurrentUser
        return home.appending(path: "Library/Voixful/Models", directoryHint: .isDirectory)
    }

    /// `~/Library/Application Support/Voixful/settings.json`.
    public static var defaultStoreURL: URL {
        let base = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? FileManager.default.homeDirectoryForCurrentUser.appending(path: "Library/Application Support")
        return base.appending(path: "Voixful/settings.json")
    }
